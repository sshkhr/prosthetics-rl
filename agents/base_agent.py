class Agent:
    ...
    def test(self, env, visualize=False, nb_max_episode_steps=300):
        """
        Run agent locally.
        """
        ...
        while not done and steps < nb_max_episode_steps:
            action = self.act(observation)
            observation, reward, done, info = env.step(action)
            total_reward += reward
            steps += 1

        print('[test] Total Reward of \'{}\': {}'.format(type(self).__name__, total_reward))
